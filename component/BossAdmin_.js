...
const BossAdmin_ = () => {
    ...
    useEffect(() => {
        ...
        socket.on("add-approve-admin", (data) => {
    ...
    const getReserveAll = async (value) => {
        ...
        try {
            let res = await axios.get(`${BASE_URL}/get-approve-boss-admin/${tmp_value}`, { headers: { "token": token } })
            setData(res.data)
        } catch (error) {
            console.log(error)
        }
    ...
    const submit = async (id, dept) => {
        ...
        try {
            let res = await axios.post(`${BASE_URL}/add-approve-boss-admin`, post, { headers: { "token": token } })
            getReserveAll()
            socket.emit('boss-admin-approve', dept)
        } catch (error) {
            console.log(error)
        }
    }
    ...
    return (
        <div>
            <div className="intro-y    h-10  mt-5">
                <div className="flex  ">
                    <UserCheck className="top-menu__sub-icon " size={32} />
                    <span className="text-3xl  truncate ml-4">หัวหน้า</span>
                </div>
                <br />
            </div>
